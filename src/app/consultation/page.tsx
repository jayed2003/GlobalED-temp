...
export default async function ConsultationPage({
  ...
  // Map URL slugs to the display values used by the form's select options
  const course = courseSlug ? courses.find((c) => c.slug === courseSlug)?.title : undefined;
  const destination = destinationSlug
    ? destinations.find((d) => d.slug === destinationSlug)?.name
    : undefined;
        ...
        <Container className="grid gap-12 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3">
            <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
              ...
              <div className="mt-6">
                <ConsultationForm defaultCourse={course} defaultDestination={destination} />
              </div>
            </div>
          </div>
        </Container>
